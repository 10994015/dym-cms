<div id="setSubaccount" class="app" wire:ignore>
    ...
    <div class="main-content">
        ...
        <table class="table table-striped table-bordered">
            <tbody>
              ...
              <tr>
                <th></th>
                <td><a href="/changeProxyPassword/{{$sub_id}}" class="btn btn-primary">密碼修改</a></td>
              </tr>
              @if(Auth::user()->highest_auth === 1)
              <tr>
                <th></th>
                <td><button type="button" class="btn btn-danger bg-danger float-right" id="deleteProxyBtn" >刪除子帳號</button></td>
              </tr>
              @endif
              <tr>
                <td colspan="2">
    ...
    <script>
        ...
        window.addEventListener("deleteFailFn", ()=>{
            alert("刪除失敗!!");
        })
        // deleteFailFn
        ...
        deleteProxyBtn.addEventListener('click', ()=>{
          if(confirm('確定要刪除嗎?刪除後就無法復原')){
            if(confirm('再次確認')){
              window.Livewire.emit('deleteSubaccount');
            }
          }
        });
    </script>
